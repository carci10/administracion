...
if ($con==$contra1 && $usuario==$usua){ 
  ...
  if ($tipousu==2){
    ...
    $_SESSION['usuario']=$usuario;
	  ...
	  $resultado3 = mysqli_query($conexion,"SELECT Codigo FROM control_escolar.usuarios WHERE NombreUsuario='$usuario' and PassUsuario='$contra1'");
    $id = $resultado3->fetch_array(MYSQLI_ASSOC);
    $id_profesor = $id["Codigo"];
