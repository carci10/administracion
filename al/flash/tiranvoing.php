...
<br /><img src="images/inscripcion.jpg" width="521" height="53" /></p>
...
$consulta=mysql_db_query("sicoes","select * from parcial");
...
if(strlen($nombre)==7)
{
$aux=substr($nombre,2,7);//para los chicos que tiene b en su folio
$nomb=$aux;
$consultal=mysql_db_query("sicoesnvo","select Matricula from usuarios where Usuario='$nombre'");
$arreglo=mysql_fetch_row($consulta1);
$matricula=$arreglo[0];
$conect=odbc_connect("sicoesnvo","","");//verificar que los campos coincidan
$consulta=mysql_db_query("sicoes", "select * from nvo_ingreso where FOLIO='$nomb'");//selecciona los grupos del alumno     posible cambio para los alumnos con "b"
...
}//aqui termina el segmento de la letra "b"
}




$aux=substr($nombre,2,6);
$nomb=$aux;
...
$conect=odbc_connect("sicoesnvo","","");//verificar que los campos coincidan
$consulta=mysql_db_query("sicoes", "select * from nvo_ingreso where FOLIO=$nombre");//selecciona los grupos del alumno
//$consulta="select Grupo from TiraMat where Matricula='$matricula' ORDER BY Grupo ;";//selecciona los grupos del alumno
while ($arreglo=mysql_fetch_array($consulta))
{
$grupo=$arreglo['GRUPO'];
...
echo "<table ><tr bgcolor=green><td>Semestres</td><td>Carreras</td><td>Grupo</td><td>Turno</td></tr>
<tr><td>$sem</td><td>$carrera</td><td>$grupo</td><td>$turno</td></tr></table><br>";
echo "<CENTER><strong>MATERÍAS A CURSAR</strong>";
$consulta3=mysql_db_query("sicoesnvo","select MATERIA,Nombre from sicoesnvo.Materias where Carrera='$carr' and Semestre='$sem'");

echo "</p> <form method=post action=selectnvoing.php>";
echo "<table  align=center><tr bgcolor=green ><td><div align=center>Semestre</div></td><td><div align=center>Curso</div></td><td><div align=center>Clave</div></td><td><div align=center>Materia</div></td><td><div align=center>Seleccionar</div></td></tr>";
$id=1;
echo "entra qui";
$consulta=$sicoesnvo->query("select MATERIA,Nombre from sicoesnvo.Materias where Carrera='$carr' and Semestre='$sem'");
