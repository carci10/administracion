...
$dig6= substr($mat,5,1);
$dig7= substr($mat,6,1);
$ultimo=$dig1+$dig2+$dig3+$dig4+$dig5+$dig6+$dig7+$num3;

if ($ultimo>=10)
{
	if($ultimo>=100)
	{
		$ultimo2= substr($ultimo,2,1);
		echo $añoserv.$per.$num3.$ultimo2;
	}
	else
	{
		$ultimo3= substr($ultimo,1,1);
		echo $añoserv.$per.$num3.$ultimo3;
	}
}
else
{
echo $añoserv.$per.$num3.$ultimo;
}

/*
mysql_connect("localhost", "tesi", "sicoes");
$consulta=mysql_db_query("sicoes", "select * from nvo_ingreso order by matricula asc");
while($arreglo=mysql_fetch_array($consulta))
{
	$mat=$arreglo['matricula'];
}
...
$año=$añoserv;
    ...
    $consulta2=mysql_db_query("sicoes", "select * from parcial");
	while($arreglo=mysql_fetch_array($consulta2))
	{
